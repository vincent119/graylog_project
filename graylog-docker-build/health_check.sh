...
if [[ -f "${GRAYLOG_HOME}"/Data/graylog/config/graylog.conf ]]
then
	# try to grep the variable from a mounted configuration
	http_publish_uri=$(grep "^http_publish_uri" "${GRAYLOG_HOME}"/Data/graylog/config/graylog.conf | awk -F '=' '{print $2}' | awk '{$1=$1};1')
	http_bind_address=$(grep "^http_bind_address" "${GRAYLOG_HOME}"/Data/graylog/config/graylog.conf | awk -F '=' '{print $2}' | awk '{$1=$1};1')
	http_enable_tls=$(grep "^http_enable_tls" "${GRAYLOG_HOME}"/Data/graylog/config/graylog.conf | awk -F '=' '{print $2}' | awk '{$1=$1};1')
